...
export class GithubStrategy extends Strategy<User> {
  constructor() {
    super('github')
  }
  ...
  async validation(request: FastifyRequest | IncomingMessage) {
    try {
      ...
      const token = request.headers['authorization']?.replace('Github', '').trim()
      ...
      const { id } = userData as GithubUser
      const user = await User.findOneBy({ idGithub: id })
